  ...
  return (
    <>
      <nav className="fixed top-0 left-0 w-full backdrop-filter backdrop-blur-lg bg-opacity-30 bg-[#0b0314] h-[100px] mt-4 text-white z-50 border border-none rounded-3xl px-4">
          ...
          <div className="hidden lg:flex gap-4 mr-6">
            <button
              className={`
                ${
                  isSubscribed
                    ? "bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 border-yellow-400"
                    : "bg-gradient-to-r from-red-500 to-red-600 border-red-500"
                } 
                text-white border-2 px-8 py-2 rounded-3xl p-[2px] m-1
                transition-all duration-300 ease-in-out
                shadow-lg hover:scale-105 relative overflow-hidden`}
              onClick={isSubscribed ? null : openSubscribeModal}
            >
              <span className="relative z-10">{isSubscribed ? "Subscribed to Newsletter" : "Subscribe to Newsletter"}</span>
              <span className="absolute inset-0 bg-gradient-to-r from-transparent via-white to-transparent opacity-20 transform -skew-x-12 transition-transform duration-1000 ease-out animate-shine"></span>
            </button>
          </div>
        ...
        {/* Mobile Menu */}
        <div
          className={`lg:hidden fixed inset-y-0 right-0 w-64 bg-[#3D1C67] transform ${isMenuOpen ? "translate-x-0" : "translate-x-full"} transition-transform duration-300 ease-in-out z-50 flex flex-col justify-start pt-20`}
        >
      ...
      >
        <Box className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-md">
          <div className="bg-[#0b0314] backdrop-filter backdrop-blur-lg p-8 rounded-lg shadow-lg w-full border border-white text-white">
            ...
            <h2 className="text-3xl font-bold mb-6 text-center" id="subscribe-modal-title">
              Subscribe
            </h2>
            {!isSuccess ? (
              <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
                ...
                <div className="flex flex-col items-center">
                  <DotLottieReact
                    src="https://lottie.host/9b752056-6934-4a2a-9a22-5eb230a5b519/1YyyJZB4rf.lottie"
                    loop
                    autoplay
                    style={{ width: "300px", height: "300px" }}
                  />
                  <p className="text-center mt-4 text-lg font-semibold">Subscription Successful!</p>
                  <p className="text-center mt-2 text-sm">This modal will close in a few seconds.</p>
                </div>
              )}
          </div>
        </Box>
